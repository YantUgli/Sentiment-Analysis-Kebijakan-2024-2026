...
# Pembagian data per-periode (era kepemimpinan) untuk analisis sentimen
# temporal. 2026 sengaja mayoritas (korpus + era terkini); 2024 & 2025
# diberi kuota cukup agar perbandingan antar-era punya massa memadai.
# Query yang SAMA dipakai di tiap periode agar komparabel.
PERIODS = [
...
def _month_windows(year):
    """Daftar jendela (since, until) bulanan untuk satu tahun, tidak
    melampaui hari ini."""
...
def bangun_rencana_per_periode():
    """Untuk tiap periode: query x jendela bulanan since:..until:..,
    di-shuffle deterministik agar sebaran bulan merata sejak awal."""
...
def load_env():
    """Baca satu atau lebih akun dari .env.

    Akun pertama tanpa suffix (X_USERNAME, X_PASSWORD, ...).
    Akun berikutnya pakai suffix angka: X_USERNAME_2, X_AUTH_TOKEN_2, dst.
    twscrape akan merotasi antar-akun sehingga rate-limit per-akun
    (jeda ~13 menit) nyaris hilang seiring bertambahnya akun.
    """
...
def polite_sleep(low=0.5, high=1.5):
    # Jeda antar-query dibuat minimal: bottleneck sebenarnya adalah
    # rate-limit X (twscrape sudah menunggu otomatis saat kuota habis),
    # jadi sleep besar hanya membuang jatah request saat kuota masih ada.
...
async def main():
    ...
    def pilih_periode():
        """Pilih periode paling tertinggal (fraksi kuota terkecil) yang
        masih punya kuota tersisa dan sisa rencana query."""
        kandidat = [
